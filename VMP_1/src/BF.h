...
struct BFsolution_st
{
    vector<BFone_pm_st> pm_in_sol; //
    float fit_sol;
    int requested_net;
};

// first shuffle the VM list randomly, then insert each VM into the first PM with sufficient remaining space.
class BF // best-fit decreasing
{
public:
    BF();
    BF(vector<pm> pm_list, vector<vm> vm_list, float RpVal, float RmVal, float PVal, std::ofstream &fileName, bool originalBF, unsigned int iterationForAvg); //,int vm_to_vm_comm[][NUM_SERVICES*MAX_VM_PER_SERVICE]);
    virtual ~BF();
    void initialize();
    unsigned int findBestPM(vm theVM, vector<pm> PMList);
    int *hosted_vms;
    ...
    void initialize_free_cap_pmsBF(vector<pm> pm_list);
    void update_free_cap_pmsBF(int cap_cpu, int cap_mem, int cap_net, int pm, int backward_modification_traffic);
    int num_active_pmsBF(vector<vm> vm_list);
