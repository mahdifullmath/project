...
struct RFsolution_st
{
    vector<RFone_pm_st> pm_in_sol; //
    float fit_sol;
    int requested_net;
};

// first shuffle the VM list randomly, then insert each VM into the first PM with sufficient remaining space.
class RF // best-fit decreasing
{
public:
    RF();
    RF(vector<pm> pm_list, vector<vm> vm_list, float RpVal, float RmVal, float PVal, std::ofstream &fileName, bool originalRF, unsigned int iterationForAvg); //,int vm_to_vm_comm[][NUM_SERVICES*MAX_VM_PER_SERVICE]);
    virtual ~RF();
    void initialize();
    vector<vm> shuffleVMList(vector<vm> vm_list);
    int *hosted_vms;
    ...
    void initialize_free_cap_pmsRF(vector<pm> pm_list);
    void update_free_cap_pmsRF(int cap_cpu, int cap_mem, int cap_net, int pm, int backward_modification_traffic);
    int num_active_pmsRF(vector<vm> vm_list);
